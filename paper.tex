...
This value is roughly equal to the expectation of the backoff chosen in line $18$ in Algorithm \ref{alg:CSMA_CA}.
...
\While{the device is on}
{
  ...
  \While{there is a packet to transmit}{
    ...
    \eIf{success}{
      %\tcc{Random backoff.}
      \colorbox{yellow}{$b \leftarrow (2^{s}\rm{CW}_{min})/2$} \;
    }
    {
      Discard packet\;
      $b \leftarrow \mathcal{U}[0, 2^s \rm{CW}_{min}-1]$\;
...
An example of the operation of CSMA/ECA with hysteresis is presented in Fig. \ref{fig:csma_eca_different_backoff}.
There are four stations in the example and the transmissions of those stations are represented as rounded boxes.
The small numbers are the value of the backoff counter of the stations, that decrement in every slot boundary.
